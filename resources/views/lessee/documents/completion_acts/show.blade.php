@extends('layouts.app')

@section('content')
<div class="container py-4">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">
                        <i class="fas fa-file-contract me-2"></i>Акт выполненных работ #{{ $actData['id'] }}
                    </h4>
                </div>
                <div class="card-body">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <h5>Информация о заказе</h5>
                            <p><strong>Номер заказа:</strong> #{{ $actData['parent_order_id'] ?? 'Не указан' }}</p>
                            <p><strong>Исполнитель:</strong> Платформа</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Дата акта:</strong> {{ $actData['act_date'] }}</p>
                            <p><strong>Период оказания услуг:</strong> {{ $actData['service_period'] }}</p>
                        </div>
                    </div>

                    <div class="row mb-4">
                        <div class="col-md-6">
                            <h5>Выполненные работы</h5>
                            <p><strong>Отработано часов:</strong> {{ $actData['total_hours'] }}</p>
                        </div>
                        <div class="col-md-6">
                            <h5>Финансовая информация</h5>
                            <p><strong>Ставка:</strong> {{ number_format($actData['hourly_rate'], 2) }} ₽</p>
                            <p><strong>Итоговая сумма:</strong> {{ number_format($actData['total_amount'], 2) }} ₽</p>
                        </div>
                    </div>

                    <!-- Детальная сводка по дням (для актов, если нужно) -->
                    @if($completionAct->waybill && $completionAct->waybill->shifts->count() > 0)
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <h5>Детальная сводка по дням</h5>
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th>Дата</th>
                                            <th>Отработано часов</th>
                                            <th>Объект</th>
                                            <th>Описание работ</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($completionAct->waybill->shifts as $shift)
                                        <tr>
                                            <td>{{ $shift->shift_date->format('d.m.Y') }}</td>
                                            <td>{{ $shift->hours_worked }}</td>
                                            <td>{{ $shift->object_name ?? 'Не указано' }}</td>
                                            <td>{{ $shift->work_description ?? 'Не указано' }}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                    @endif

                    <div class="d-flex gap-2">
                        <a href="{{ route('documents.completion-act.download', $actData['id']) }}"
                        class="btn btn-primary">
                            <i class="fas fa-download me-1"></i>Скачать PDF
                        </a>

                        <a href="{{ route('lessee.orders.show', $actData['parent_order_id'] ?? '#') }}"
                        class="btn btn-outline-secondary">
                            <i class="fas fa-external-link-alt me-1"></i>Перейти к заказу
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
