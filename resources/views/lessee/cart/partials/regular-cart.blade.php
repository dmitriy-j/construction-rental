...
@else
    ...
    <div class="alert alert-info mb-4">
        <i class="bi bi-info-circle me-2"></i>Рассчитанная стоимость является ориентировочной. Окончательная сумма будет определена в акте выполненных работ на основании фактического времени использования техники.
    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @php $cartTotalDelivery = 0; @endphp
                        @foreach($cart->items as $item)
                            @php
                                $locFrom = optional($item->rentalTerm->equipment->location);
                                $deliveryFrom = $locFrom->name ?: $locFrom->address ?: 'Не указано';
                                $deliveryTo = $item->address ?: 'Не указано';
                                $cartTotalDelivery += $item->delivery_cost;
                            @endphp
                            <tr>
                                <td>
                                ...
                                <td class="text-center">
                                    <span class="badge bg-primary rounded-pill px-3 py-2">{{ $item->period_count }} ч</span>
                                </td>
                                ...
                                <td class="text-center">
                                    @if($item->delivery_cost > 0)
                                        <button type="button" class="btn btn-sm btn-outline-primary"
                                                ...
                                                data-delivery-from="{{ $deliveryFrom }}"
                                                data-delivery-to="{{ $deliveryTo }}"
            ...
            <div class="row justify-content-end">
                <div class="col-md-6">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-medium">{{ number_format($cartTotalDelivery, 2) }} ₽</span>
                    </div>
                    <hr>
                    <div class="d-flex justify-content-between fs-5">
                        <span class="fw-bold">Всего к оплате:</span>
                        <span class="fw-bold text-primary">{{ number_format($totalRental + $cartTotalDelivery, 2) }} ₽</span>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const getSelectedItems = () => [...document.querySelectorAll('.item-checkbox:checked')].map(el => el.value);

    function updateSelectedItems() {
        const v = JSON.stringify(getSelectedItems());
        const ci = document.getElementById('selected-items');
        const bi = document.getElementById('selected-items-input');
        if (ci) ci.value = v;
        if (bi) bi.value = v;
    }
    ...
    function initSelectAll() {
        ...
        document.addEventListener('change', function(e) {
            if (e.target.classList.contains('item-checkbox')) {
                const cs = document.querySelectorAll('.item-checkbox');
                el.checked = [...cs].every(c => c.checked);
                updateSelectedItems();
            }
        });
    }
    ...
    function initRemoveSelected() {
        ...
        btn.addEventListener('click', async function() {
            ...
            if (!sel.length) {
                Swal.fire({ icon: 'warning', title: 'Не выбраны элементы' });
                return;
            }
            const r = await Swal.fire({ title: 'Удалить ' + sel.length + ' позиций?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Да, удалить' });
            if (!r.isConfirmed) return;
            const t = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            let s = 0;
            for (const id of sel) {
                try {
                    const res = await fetch('/api/cart/' + id, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': t } });
                    if ((await res.json()).success) s++;
                } catch(e) {}
            }
            await Swal.fire({ icon: 'success', title: 'Удалено ' + s + ' из ' + sel.length, timer: 1500, showConfirmButton: false });
            location.reload();
        });
    }
    ...
    function initPopovers() {
        document.querySelectorAll('[data-bs-toggle="popover"]').forEach(el => {
            new bootstrap.Popover(el, {
                html: true, title: 'Детали доставки',
                content: '<div><strong>Откуда:</strong> ' + (el.dataset.deliveryFrom || '—') + '</div><div><strong>Куда:</strong> ' + (el.dataset.deliveryTo || '—') + '</div><div><strong>Стоимость:</strong> ' + (el.dataset.deliveryCost || '0') + ' ₽</div>',
                trigger: 'hover focus'
            });
        });
    }
