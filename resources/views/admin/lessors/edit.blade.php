...
@section('title', 'Редактирование арендодателя')

@section('content')
<div class="container-fluid">
    <div class="row mb-3">
        <div class="col">
            <h2>Редактирование: {{ $lessor->legal_name }}</h2>
        </div>
        <div class="col-auto">
            <a href="{{ route('admin.lessors.show', $lessor) }}" class="btn btn-outline-info">
                <i class="bi bi-eye"></i> Просмотр
            </a>
            <a href="{{ route('admin.lessors.index') }}" class="btn btn-outline-secondary">
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.lessors.update', $lessor) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="legal_name" class="form-control @error('legal_name') is-invalid @enderror"
                               value="{{ old('legal_name', $lessor->legal_name) }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="legal_type" class="form-select">
                            <option value="ooo" {{ old('legal_type', $lessor->legal_type) == 'ooo' ? 'selected' : '' }}>ООО</option>
                            <option value="ip" {{ old('legal_type', $lessor->legal_type) == 'ip' ? 'selected' : '' }}>ИП</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="director_name" class="form-control"
                               value="{{ old('director_name', $lessor->director_name) }}">
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="inn" class="form-control @error('inn') is-invalid @enderror"
                               value="{{ old('inn', $lessor->inn) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="legal_address" class="form-control" rows="2">{{ old('legal_address', $lessor->legal_address) }}</textarea>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Фактический адрес</label>
                        <textarea name="actual_address" class="form-control" rows="2">{{ old('actual_address', $lessor->actual_address) }}</textarea>
                    </div>
                    ...
                    <div class="col-md-12 mt-4">
                        ...
                        <a href="{{ route('admin.lessors.index') }}" class="btn btn-outline-secondary">Отмена</a>
                    </div>
                </div>
            </form>
        </div>
